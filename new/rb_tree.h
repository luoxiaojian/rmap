...
class _Rb_tree {
 ...
 protected:
  _Link_type& _M_root() { return this->alloc_._M_header().parent(); }

  _Const_Link_type _M_root() const { return this->alloc_._M_header().parent(); }

  _Link_type& _M_leftmost() { return this->alloc_._M_header().left(); }

  _Const_Link_type _M_leftmost() const {
    return this->alloc_._M_header().left();
  }

  _Link_type& _M_rightmost() { return this->alloc_._M_header().right(); }

  _Const_Link_type _M_rightmost() const {
    return this->alloc_._M_header().right();
  }

  _Link_type _M_begin() { return this->alloc_._M_header().parent(); }

  _Const_Link_type _M_begin() const {
    return this->alloc_._M_header().parent();
  }

  _Link_type _M_end() { return &(this->alloc_._M_header()); }

  _Const_Link_type _M_end() const { return &(this->alloc_._M_header()); }
 ...
 private:
  pair<_Link_type, _Link_type> _M_get_insert_unique_pos(const key_type& __k);
  ...
  // pair<_Link_type, _Link_type> _M_get_insert_equal_pos(const key_type& __k);

  // pair<_Link_type, _Link_type> _M_get_insert_hint_unique_pos(
  // const_iterator __pos, const key_type& __k);

  // pair<_Link_type, _Link_type> _M_get_insert_hint_equal_pos(
  // const_iterator __pos, const key_type& __k);

  // iterator _M_insert_lower(_Link_type __y, const value_type& __v);
  ...
  */
  ...
  iterator begin() { return iterator(this->alloc_._M_header().left()); }

  const_iterator begin() const {
    return const_iterator(this->alloc_._M_header().left());
  }

  iterator end() { return iterator(&this->alloc_._M_header()); }

  const_iterator end() const {
    return const_iterator(&this->alloc_._M_header());
  }

  bool empty() const {
    return alloc_.Size() == 0;
...
template <typename _Key, typename _Val, typename _KeyOfValue, typename _Compare,
          typename _Alloc>
pair<typename _Rb_tree<_Key, _Val, _KeyOfValue, _Compare, _Alloc>::_Link_type,
...
void _Rb_tree<_Key, _Val, _KeyOfValue, _Compare>::_M_erase_aux(
  ...
  _Link_type __y = static_cast<_Link_type>(_Rb_tree_rebalance_for_erase(
      const_cast<_Link_type>(__position._M_node), this->_M_impl._M_header));
  _M_destroy_node(__y);
  --_M_impl._M_node_count;
}
